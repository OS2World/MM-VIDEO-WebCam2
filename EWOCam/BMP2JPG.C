/*************************************************************************
 * File Name    :  MMIO\DATATRAN\main.c
 *
 * Note: Copy the flamingo.bmp or some other .bmp file from the 
 *       \os2\bitmap directory to the current directory.
 *************************************************************************/
...
LONG           lNumBytesRead = 0;
    ...
    hmmioFrom = mmioOpen(infile, &mmioinfo, MMIO_READ);
    /* Check for error */
    if (!hmmioFrom) {
        /* error */
        goto end;
    ...
    hmmioTo = mmioOpen(outfile, &mmioinfo, MMIO_CREATE+MMIO_WRITE);
